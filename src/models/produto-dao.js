class ProdutoDao {
    ...
    adiciona(produto){
        return new Promise( (resolver, rejeitar) => {
            const sql = `INSERT INTO PRODUTOS(nome,laboratorio,preco,numeroProduto, quantidade, descricao) values (?,?,?,?,?,?)`

            this._conexao.query(sql,[produto.nome,produto.laboratorio,produto.preco,produto.numeroProduto,produto.quantidade, produto.descricao],
                function(msgErro) {
                    if(msgErro){
                        console.log(msgErro)
